...
A `MetaTransactionExecuted <../basics/events.html#metatransactionexecuted>`_ event is emitted for each mtx on success. The ``returnResult`` contains the raw return data for the executed function This call will revert if one of the ``mtxs`` reverts. Any excess Ether will be refunded to the ``msg.sender``.
